...
export function getUniV2PairShare(pair: UniswapV2Pair): BigDecimal {
  ...
  const wallet1Share = pair.balanceOf(Address.fromString(AVAX_TREASURY_1)).toBigDecimal();
  const wallet2Share = pair.balanceOf(Address.fromString(AVAX_TREASURY_2)).toBigDecimal();
  const share = wallet1Share.plus(wallet2Share);
  const proportion = share.div(totalSupply);
  log.info("UniV2 SHARE PROPORTION" + proportion.toString(), []);
  return proportion;
}
